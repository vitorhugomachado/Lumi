...
export async function account(request: Request) {
  ...
  const result = await db().query(
    "SELECT a.id,a.email,a.is_guest,a.display_name,a.email_verified_at,s.remember_me FROM lumi_sessions s JOIN lumi_accounts a ON a.id=s.account_id WHERE s.token_hash=$1 AND s.expires_at>now()",
    [digest(token)],
  );
  if (!result.rows[0])
    throw new ApiError(
